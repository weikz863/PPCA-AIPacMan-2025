...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict = {PAC: [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)], 
                           ...
                           OBS0: list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1)),
                           OBS1: list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1)),}
    "*** END YOUR CODE HERE ***"
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for eliminateVariable in eliminationOrder:
            ...
            if len(joinedFactor.unconditionedVariables()) >= 2:
                joinedFactor = eliminate(joinedFactor, eliminateVariable)
                currentFactorsList.append(joinedFactor)
        fullJoint = joinFactors(currentFactorsList)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            self.beliefs[position] *= self.getObservationProb(observation, \
                    gameState.getPacmanPosition(), position, self.getJailPosition())
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        newBelief = defaultdict(float)
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for (newPos, prob) in newPosDist.items():
                newBelief[newPos] += self.beliefs[oldPos] * prob
        for position in self.allPositions:
            self.beliefs[position] = newBelief[position]
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        priorDistribution = self.getBeliefDistribution()
        postDistribution = DiscreteDistribution()
        for (pos, prob) in priorDistribution.items():
            postDistribution[pos] = prob * self.getObservationProb(observation, pacmanPos, pos, jailPos)
        if postDistribution.total() == 0:
            self.initializeUniformly(gameState)
            postDistribution = self.getBeliefDistribution()
        self.particles = [postDistribution.sample() for _ in range(self.numParticles)]
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        postDistribution = DiscreteDistribution()
        for particle in self.particles:
            particleDistribution = self.getPositionDistribution(gameState, particle)
            for (pos, prob) in particleDistribution.items():
                postDistribution[pos] = postDistribution.get(pos, 0) + prob
        self.particles = [postDistribution.sample() for _ in range(self.numParticles * 2)]
        "*** END YOUR CODE HERE ***"
